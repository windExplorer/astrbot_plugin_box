...
- 自动展示新群友的资料卡片（需在配置里添加群聊白名单），退群/被踢成员的资料卡也会区分展示（角标标明类型）
- 入群卡可附带 @新成员 的欢迎语文本（模板可配置，支持 AI 个性化欢迎语与私聊群规）
...
在插件配置的「AI 分析」中可分别开启三项（消耗 token，入群自动展示时同样触发）：

- **头像分析**：LLM 看头像画面写一句点评
- **签名解读**：LLM 解读个性签名（对方没写签名时不展示）
- **综合锐评**：LLM 整合全部公开资料和头像写一句锐评

群聊卡片还会显示该成员的入群排位（本群第几位加入），并默认用本地数据库记录群成员入群、退群的精确时间（到秒）——退群成员的卡片也能显示加群/退群时间。
